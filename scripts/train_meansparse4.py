...
def main():
        ...
        for name, module in model.named_modules():
            if isinstance(module, MeanSparse):
                ...
                if f"{name}.batch_num" not in state_dict:
                    state_dict[f"{name}.batch_num"] = th.tensor(80072, dtype=module.batch_num.dtype, device=module.batch_num.device)
        return state_dict
    ...
    meansparse_classifiers = [create_and_initialize_meansparse_classifier(args) for _ in range(1)]
    ...
    def collect_all_data_using_next(data_loader):
        all_batches = []
        all_labels = []
        count=0
        while True:
    ...
    def save_data(batch, labels, filename='saved_data.pth'):
        th.save((batch, labels), filename)
        print(f'Saved data to {filename}')

    def load_saved_data(filename='saved_data.pth'):
        batch, labels = th.load(filename)
    ...
    while (count<80072):
        ...
        # if timestamp in sub_t.tolist():
        #     state_dict = meanspare_classifier.state_dict()
        #     layer_name = 'module.input_blocks.1.0.meansparse'  # Replace with actual layer name
        ...
        #     running_mean = state_dict[running_mean_key].cpu().numpy()
        #     running_var = state_dict[running_var_key].cpu().numpy()
            
        #     running_means[timestamp].append(running_mean)
        #     running_vars[timestamp].append(running_var)
    ...
    save_classifiers(meansparse_classifiers_ddp, 'meansparse_classifiers_t600.pth')


    # Compute and print running statistics
    def compute_and_print_statistics(timestamps, running_means, running_vars):
        for timestamp in timestamps:
            if running_means[timestamp]:
                print(f"Timestamp {timestamp}:")
                print(f"Running Mean: {running_means[timestamp]}")
                print(f"Running Variance: {running_vars[timestamp]}")

    # compute_and_print_statistics(timestamps, running_means, running_vars)
...
# # Example code to generate a batch of images (replace this with your actual batch)
# # Simulate a batch with 4 images of size 3x64x64
# batch_size, channels, height, width = 10, 3, 64, 64
...
def transform_for_plotting(tensor):
    np_img = tensor_to_np(tensor)
    
    # Clip data to valid range for imshow
    if np_img.dtype == np.float32 or np_img.dtype == np.float64:
        np_img = np.clip(np_img, 0, 1)  # Clip floats to [0, 1]
    elif np_img.dtype == np.uint8:
        np_img = np.clip(np_img, 0, 255)  # Clip integers to [0, 255]
    
    return np.transpose(np_img, (1, 2, 0))

# Plot images
def plot_batch(batch):
    batch_np = tensor_to_np(batch)  # Convert to NumPy array
    batch_np = [transform_for_plotting(img) for img in batch_np]  # Transform each image for plotting
    
    # Create subplots
    fig, axes = plt.subplots(1, len(batch_np), figsize=(12, 4))
